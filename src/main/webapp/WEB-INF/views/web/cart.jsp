    ...
    <div class="container">
        <div class="row">
            <c:if test="${not empty status }">
                <div class="col-12" style="color:#ff084e;">*${status}</div>
            </c:if>
            <div class="col-12">
                            ...
                            <c:forEach var="item" items="${ShoppingCart}">
                                <tr>
                                    <td class="cart_product_img d-flex align-items-center">
                                        <a href="#">
                                            <img src="<c:url value='/template/web/img/product-img/${item.value.product.photo}'/>" alt="Product">
                                        </a>
                                        <h6>${item.value.product.name}x
                                            ${item.value.product.size}</h6>
                                    </td>
                                    <td class="price"><span>
                                            <fmt:formatNumber type="number" maxFractionDigits="3"
                                                              value="${item.value.product.price}" /> ₫</span>
                                    </td>
                                    <td class="qty">
                                    ...
                                    <td class="total_price">
                                        <span><fmt:formatNumber type="number" maxFractionDigits="3"
                                                                value="${item.value.totalPrice}" /> ₫</span>
                                    </td>
                                    <td class="delete_product">
                                        <a href="<c:url value="/cart/delete-cart/id=${item.key}" />"> Xóa </a>
                                    </td>
                                </tr>
                            </c:forEach>
