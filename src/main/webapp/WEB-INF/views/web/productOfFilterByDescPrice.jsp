							...
							<nav aria-label="Page navigation">
									...
									<ul class="pagination pagination-sm">
										...
										<li class="page-item"><a class="page-link"
											href="<c:url value='/product/sort-by=price_descending/page=1'/>">&laquo;</a>
											<c:forEach var="item" begin="1"
												end="${ paginateInfo.totalPage }" varStatus="loop">
												<c:if test="${ (loop.index) == paginateInfo.currentPage }">
													<li class="page-item active"><a class="page-link"
														href="<c:url value='/product/sort-by=price_descending/page=${ loop.index }'/>">${ loop.index }</a></li>
												</c:if>
												<c:if test="${ (loop.index) != paginateInfo.currentPage }">
													<li class="page-item"><a class="page-link"
														href="<c:url value='/product/sort-by=price_descending/page=${ loop.index }'/>">${ loop.index }</a></li>
										...
										<li class="page-item"><a class="page-link"
											href="<c:url value='/product/sort-by=price_descending/page=${ paginateInfo.totalPage }'/>">&raquo;</a></li>
									</ul>
								</c:if>
							</nav>
						</div>
